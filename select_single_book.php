...
try {
    ...
    $statement= $db->prepare('select * from books where id=:book and isDelete=0 ');
...
try {
   // $dsn = 'mysql:dbname=library;host=localhost';
	...
	$statement= $db->prepare('select * from record where booksid=? ');
    $statement->execute(array($_POST['book']));
	//$result2 = $db->query('SELECT * FROM record where where booksid=?')->fetchAll();
